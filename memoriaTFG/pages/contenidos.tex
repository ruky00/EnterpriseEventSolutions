...
\tutor{Faltan los requisitos aquí ...}
...
\tutor{La figura de la arquitectura no es correcta. Te pasaré otras memorias para que veas como debería ser. No está mal dividirla en componentes, pero en tu caso, no es lo más adecuado. Cuando la cambies, deberás describirla mejor.}
...
\begin{itemize}
    ...
    \tutor{La justificación de las tecnologías va en la sección anterior, aquí todo esto sobra.}
\end{itemize}
...
Cada uno de los componentes definidos en la Imagen\tutor{Usa Figura en lugar de Imagen} \ref{fig:mvc_architecture} será desglosado en las próximas secciones para explicar detalladamente el contenido de 
la aplicación.
...
\begin{figure}[h]
    ...
    \caption{Modelo E-R de la BD\tutor{La Figura es correcta, pero se sale de los márgenes. Trata de colocar alguna de las tablas debajo con la herramienta que hayas utilizado.}}
...
Además he implementado otras configuraciones para añadir una capa extra de seguridad a la aplicación como configuración CSRFH\tutor{Te sobra una H} y Cors \tutor{Mejor en mayúsculas, son siglas} para limitar las url 
...
\begin{figure}[h]
    ...
    \caption{Colección Postman\tutor{La figura es poco visible, apenas se lee nada. Salvo que la puedas mejorar, yo la eliminaba, no aporta mucho.}}
    \label{fig:postman}
\end{figure}
...
\tutor{Me temo que no aporta nada, no hay legibilidad alguna. Puedes cambiarlo por un par de capturas de pantalla relevantes o simplemente eliminarlo.}
\begin{figure}[h]
    \centering
    \rotatebox{90}{
...
Para desplegar la aplicación se ha utilizado Docker, creando una imagen de la aplicación y lanzandola\tutor{lanzándola} junto con un contenedor de MySQL para la persitencia\tutor{persistencia} se puede ejecutar
la aplicación sin enfrentarse a problemas de dependencias.\tutor{Hay una sección concreta para el despliegue, lo que te avisa que esto no debería estar aquí}
...
\tutor{Los requisitos no van aquí ... se supone que son de lo que parte tu aplicación, ¿qué sentido tiene contarlos lo último?}
